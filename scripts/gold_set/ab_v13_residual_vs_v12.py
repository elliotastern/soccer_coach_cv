#!/usr/bin/env python3
"""Rebuild strip (+ optional holdout) det caches for a checkpoint, then score vs v12.
...
OUT = ROOT / "reports/eval_match3/improve_eng_loop/ab_v13_residual_vs_v12.json"
CACHE_DIR = ROOT / "reports/eval_match3/improve_eng_loop/v13_residual_det_cache"
STRIPS = [
...
def main() -> int:
    ...
    p.add_argument(
        "--v13",
        type=Path,
        default=ROOT / "models/v13_residual_snaps/post_train/checkpoint.pth",
    )
    p.add_argument(
        "--baseline-json",
        type=Path,
        default=ROOT / "reports/eval_match3/improve_eng_loop/m1_strip_score.json",
    )
    p.add_argument("--stride", type=int, default=None, help="override strip stride")
    args = p.parse_args()
    if not args.v13.is_file():
        raise SystemExit(f"missing {args.v13}")

    model = load_ball_model(str(args.v13))
    strips_v13 = {}
    for lab in STRIPS:
        if not lab.is_file():
            continue
        ...
        cache_path = CACHE_DIR / f"det_cache_{labels['stem']}_v13_thr010.json"
        print(f"detect {labels['pack']} stride={stride} → {cache_path.name}")
        detect_strip_cache(lab, model, cache_path, stride)
        strips_v13[labels["pack"]] = score_with_cache(lab, cache_path)
    ...
    kills = []
    for name, row in strips_v13.items():
        pe = row.get("P_emit")
        if pe is not None and pe < P_EMIT_GATE:
            kills.append(f"{name} P_emit={pe}")

    payload = {
        "v13_ckpt": str(args.v13),
        "strips_v13": strips_v13,
        "baseline_strips": (baseline.get("strips") if isinstance(baseline, dict) else None),
        "p_emit_kills": kills,
