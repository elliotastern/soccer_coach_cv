#!/usr/bin/env python3
"""v14 residual specialty: leftover soft/no_det under v13 strip caches.

Resume-from-v13. Prefer v13_residual_det_cache when scoring residual frames.
Match3 120–194 held out. Do not include holdout gallery frames.
"""
...
OUT = Path("/Volumes/LaCie/Projects/Soccer project data/ball_finetune_v14_residual")
...
def resolve_det_cache(pack_dir: Path, labels: dict, spec: dict) -> Path:
    """Prefer v13 strip caches, then labels.json, then gallery fallbacks."""
...
def main() -> int:
    ...
    manifest = {
        "seed": SEED,
        "recipe": "v14_residual_fn_conf_after_v13",
        ...
        "notes": [
            "Thin residual specialty after v13 promote (leftover P8/P10 soft conf + no_det under v13 caches).",
            "Resume from v13_residual; short epoch budget (see configs/finetune_v14_residual_mac.yaml).",
    ...
    local = ROOT / "data/processed/gold_sets/ball_finetune_v14_residual_manifest.json"
