#!/usr/bin/env python3
"""v13 residual specialty: Match3 strip FN conf/localization, resume-from-v12.

Thin pack (not a full v12 redo). Residual clear frames from P8/P10 (P9 if
present) where focus det conf is soft (0.20–0.79) or missing, plus a modest
v12_hard train anchor so medium balls do not collapse. Match3 120–194 held out.
"""
...
OUT = Path("/Volumes/LaCie/Projects/Soccer project data/ball_finetune_v13_residual")
V12_CANDIDATES = [
...
def resolve_det_cache(pack_dir: Path, labels: dict, spec: dict) -> Path:
    """Prefer labels.json strip det_cache (Phase 0 / M1 scoring), then fallbacks."""
...
def main() -> int:
    ...
    manifest = {
        "seed": SEED,
        "recipe": "v13_residual_fn_conf",
        "resume_from": "models/v12_hard_snaps/post_train/checkpoint.pth",
        "train": train_stats,
        "valid": valid_stats,
        "raw_counts": {
        ...
        "notes": [
            "Thin residual specialty after Phase 0 FN triage (P8/P10 soft conf + no_det).",
            "Resume from v12_hard; short epoch budget (see configs/finetune_v13_residual.yaml).",
            "Hard blur / tiny paste only on residual strip items (2 copies each).",
            "v12_hard train subset anchors medium balls so specialty does not collapse AP.",
        ],
    }
    (OUT / "manifest.json").write_text(json.dumps(manifest, indent=2))
    local = ROOT / "data/processed/gold_sets/ball_finetune_v13_residual_manifest.json"
