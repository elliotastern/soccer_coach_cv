...
```
soccer_coach_cv/
├── apps/                   # batch_pipeline, review_dashboard, live stub
├── configs/                # Training and inference YAML
├── src/
│   ├── perception/         # Detection, tracking, team assign
│   ├── mapping/            # Pitch coordinates
│   ├── events/             # Heuristic events
│   ├── review/             # Streamlit dashboard
│   └── training/           # Training utilities
├── scripts/                # train_ball, experiments, ball_validation
├── models/                 # Checkpoints (not in Hub by default with --no-models)
...
- `configs/training_finetune.yaml` - Lower LR finetune from a checkpoint
- `configs/default.yaml` - Pipeline / Roboflow inference defaults
...
def push_to_huggingface(
    ...
    # Generate README for Hub without overwriting the repository README
    print("📝 Creating temporary README for Hub upload...")
    ...
    tmp.close()
    readme_path = Path(tmp.name)
    print("✅ Hub README prepared (repo README.md unchanged)")
