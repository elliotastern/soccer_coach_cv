...
| **Det v16** | Fuse-silent residual specialty resume-from-v14 (@285→300) | Fair A/B: P10 clear_R **0.953→0.966**, P8 **0.952→0.971**, P_emit **1.0**; holdout **0.987** (`ab_v16_residual_vs_v14.json`) | Product `ball_checkpoint` → `v16_residual_snaps` |
...
| **C3b** | **P1 hull expand** (near-touch `hull_image_points`) — kickoff P1 conf≥0.80 low_support (feet y~950–1040 @1080) | Strips/holdout held **1.0 / 0.987** (`ab_p1_hull_expand.json`) | Unlock P1 near-touch maps without refitting H |
...
| **F0** | Detect-tick **hold** (`fuse_balls_with_hold`, `HOLD_MAX_GAP=24`) — not Phase 2 RNN fusion | 2026-09-04 v16 A/B: hold 8→24 strip clear_R → **1.0 / 1.0**, P_emit **1.0**; holdout **0.987** flat (`ab_hold_max_gap_v16.json`) | Clear-R across silent ticks; F3c keeps ballcap holds from poisoning |
| **F0b** | Soft-hold **renew**: after gap &gt; 8, re-emit prev conf if high-support soft map (conf≥0.55, support≥0.50) still ≤4 m from prev | 2026-09-04 A/B: holdout clear **0.978→0.993**; strip P_emit flat; clear_R +0.004 (`ab_soft_hold_renew.json`) | Recover soft near-miss after hold expires without lowering emit 0.80 |
...
| **F3c** | Solo emit requires `support ≥ 0.50` on **P7 only** (`SOLO_STRICT_CAMS`) | 2026-09-04: all-cam 0.50 → P_emit 1.0; scoped P7-only → P10 clear_R **0.914→0.953**, P_emit **1.0** (`ab_solo_min_support_scope.json`) | Kill P7 ballcaps; keep P10 edge balls (support ~0.25–0.35) |
